...
@section('content')
    <div class="row justify-content-center">
        ...
        <div class="col-6 py-4">
            <form class="row g-4" action="{{ route('attributes_value.update',$attributeValue->id) }}" method="post">
                @csrf
                @method('PATCH')
                <div class="col-12">
                    <label for="inputTitle" class="form-label">مقدار ویژگی</label>
                    <input type="text" class="form-control" id="inputTitle" name="title"  placeholder="مقدار ویژگی..." value="{{$attributeValue->title}}">
                </div>
                <div class="col-12">
                    ...
                    <select class="form-select searchSelect mb-4" id="inputParent" name="attributes_group_id">
                        ...
                        @foreach ($attributesGroup as $attribute)
                            <option value="{{ $attribute->id }}" @if($attributeValue->attributes_group_id == $attribute->id) selected @endif>{{ $attribute->title }}</option>
                        @endforeach
                    </select>
                </div>
