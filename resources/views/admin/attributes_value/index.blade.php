...
@section('content')
    <div class="bg-white col-12 p-3 pb-5 border-start border-4 border-info right-box">
        @if (Session::has('operation_attribute_value'))
            @include('admin.partials.Alert', ['msg' => [session('operation_attribute_value')], 'status' => 'success'])
        @endif
        ...
        <table class="table">
            <thead class="bg-dark text-white">
                <tr>
                    <th>#</th>
                    <th class="fw-normal fs-18">مقدار ویژگی</th>
            ...
            <tbody>
                @foreach ($attributesValue as $key => $attribute)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $attribute->title }}</td>
                        <td>{{ $attribute->attributes_group->title }}</td>
                        <td>{{ verta($attribute->created_at)->format('H:i:s') }} -
                            {{ verta($attribute->created_at)->format('Y/m/d') }}</td>
                        <td>
                            <div class="d-flex align-items-center justify-content-center gap-2 pt-1">
                                <a href="{{ route('attributes_value.edit', $attribute->id) }}"
                                    title="ویرایش ویژگی {{ $attribute->title }}">
                                    <i class="icon-edit-1 fs-6"></i>
                                </a>
                                <form action="{{ route('attributes_value.destroy', $attribute->id) }}" method="Post"
                                    class="m-0">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-danger border-0 p-0 bg-transparent"
                                        title="حذف ویژگی {{ $attribute->title }}">
