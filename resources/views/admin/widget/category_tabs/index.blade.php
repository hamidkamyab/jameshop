...
@endsection
@section('navBtnBox')
    <a href="{{ route('category_tabs.create') }}" class="btn btn-success d-flex align-items-center">
        <i class="icon-plus"></i>
        <span>افزودن</span>
    </a>
@endsection
@section('content')
    <div class="bg-white col-12 p-3 border-start border-4 border-info right-box">
        @if (Session::has('opration_catTab'))
            @include('admin.partials.Alert', ['msg' => [session('opration_catTab')], 'status' => 'success'])
        @endif
        <table class="table">
                    ...
                    <tr class="align-middle">
                        ...
                        <td>
                            <div class="d-flex align-items-center justify-content-center gap-2 pt-1">
                                <a href="{{ route('category_tabs.edit', $catTab->id) }}" title=" ویرایش برگه {{ $catTab->title }} ">
                                    <i class="icon-edit-1 fs-6"></i>
                                </a>
                                <form action="{{ route('category_tabs.destroy', $catTab->id) }}" method="Post" class="m-0">
                                    @csrf
                                    @method('DELETE')
